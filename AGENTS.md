...
Portfolio pipeline: `demo_mode/` + `Documentation/` (screenshots in `Documentation/images/`).
On changes: update CHANGELOG and version files per the rule — do not wait to be asked.
`demo_mode/` must stay excluded from the main product production build (separate Streamlit deploy only).
